...
void GraphicalUI::setupUndoRedoShortcuts()
{
	auto shortcutUndo = new QShortcut(QKeySequence(Qt::CTRL + Qt::Key_Z), this);
	auto shortcutRedo = new QShortcut(QKeySequence(Qt::CTRL + Qt::Key_Y), this);

	connect(shortcutUndo, &QShortcut::activated, this, &GraphicalUI::handleUndoClick);
	connect(shortcutRedo, &QShortcut::activated, this, &GraphicalUI::handleRedoClick);
}
...
void GraphicalUI::showMainMenu()
{
	...
	auto buttonUndo = new QPushButton{ "Undo" };
	auto buttonRedo = new QPushButton { "Redo" };
	...
	connect(buttonUndo, &QPushButton::clicked, this, &GraphicalUI::handleUndoClick);
	connect(buttonRedo, &QPushButton::clicked, this, &GraphicalUI::handleRedoClick);
	...
	if (service.isAdmin)
	{
		mainLayout->addWidget(buttonUndo);
		mainLayout->addWidget(buttonRedo);
	}
...
void GraphicalUI::handleListClick()
{
	...
	auto tableModel = new TrenchCoatTableView { trenchCoats, this };
...
void GraphicalUI::handleUndoClick()
{
	service.undo();
}

void GraphicalUI::handleRedoClick()
{
	service.redo();
}
