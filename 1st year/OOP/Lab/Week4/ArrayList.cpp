...
void ArrayList<E>::resizeIfNeeded(int newSize)
{
	...
	E* temporaryElements = new E[memorySize];
...
void ArrayList<E>::addAt(const int& index, const E& element)
{
	...
	{
		throw "Invalid index, index out of range";
	}
	resizeIfNeeded(size + 1);
...
template<class E>
E& ArrayList<E>::find(const std::function<bool(const E&)>& selection) const
{
	for (auto i = 0; i < size; i++)
	{
		if (selection(elements[i]))
...
template<class E>
ArrayList<E>& ArrayList<E>::reversed()
{
...
template<class E>
ArrayList<E>& ArrayList<E>::filter(const std::function<bool(const E&)>& selection)
{
