...
/// O(log n)
void SortedBag::addValueToTree(Node *root, TComp value, int times) {
...
//            root->left = createNode(value);
            ...
            addValueToTree(root->left, value, times);
        }
    } else {
...
//            root->right = createNode(value);
            ...
            addValueToTree(root->right, value, times);
        }
    }
}
...
void SortedBag::addOccurrences(int nr, TComp elem) {
    ...
    if (tree == nullptr) {
        ...
        if (nr > 1) {
            addValueToTree(tree, elem, nr - 1);
        }
    } else {
        addValueToTree(tree, elem, nr);
    }
    length += nr;
}
