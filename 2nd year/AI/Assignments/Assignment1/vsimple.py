...
class Environment:
    ...
    def read_udm_sensors(self, x, y):
        ...
        while xf >= 0 and self.__surface[xf][y] == 0:
            xf = xf - 1
            readings[Direction.UP] = readings[Direction.UP] + 1

        # DOWN
        xf = x + 1
        while xf < self.__n and self.__surface[xf][y] == 0:
            xf = xf + 1
            readings[Direction.DOWN] = readings[Direction.DOWN] + 1

        # LEFT
        yf = y + 1
        while yf < self.__m and self.__surface[x][yf] == 0:
            yf = yf + 1
            readings[Direction.LEFT] = readings[Direction.LEFT] + 1

        # RIGHT
        yf = y - 1
        while yf >= 0 and self.__surface[x][yf] == 0:
            yf = yf - 1
            readings[Direction.RIGHT] = readings[Direction.RIGHT] + 1
    ...
    def image(self, colour=Color.BLUE, background=Color.WHITE):
        imagine = pygame.Surface((420, 420))
        brick = pygame.Surface((20, 20))
        brick.fill(Color.BLUE)
        imagine.fill(Color.WHITE)
